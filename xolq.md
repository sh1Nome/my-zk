...
| 指定したサービスを自動起動するように設定する         | `systemctl enable (サービス名)`, `--now` ですぐに反映      |
| 指定したサービスの自動起動の設定を無効化する         | `systemctl disable (サービス名)`, `--now` ですぐに反映     |
| 指定したサービスの自動起動の設定を無効化する         | `systemctl disable (サービス名)`                           |
